...
def train_model_for(property_, start_epoch=-1):
    ...
    t_mean, t_mad, p_mean, p_mad = mean_mad_t_p()
    ...
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=2, gamma=0.4, last_epoch=start_epoch)
    loss_l1 = nn.L1Loss()
    dataloader = {}
    ...
    def train(epoch_, loader, partition="train"):
        ...
        for i, data in enumerate(loader):
            ...
            temperature = (data['temperature_list'].view(-1, 1).to(device, dtype) - t_mean) / t_mad
            pressure = (data['pressure_list'].view(-1, 1).to(device, dtype) - p_mean) / p_mad
            ...
            if partition == 'train':
                ...
                loss = loss_l1(data_mad * prediction + data_mean, label)
                prefix = ">> %s \t" % partition
            ...
            if i % args.log_interval == 0:
                print(prefix + "Epoch %d \t Iteration %d \t loss %.4f" % (
                    epoch_, i, sum(res_['loss_arr']) / res_['counter']))
