...
from B_Train.BC_diffusion.train_test import train_epoch, test, analyze_and_save
...
parser.add_argument('--batch_size', type=int, default=64)  # 128 for QM, 64 for GEOM
parser.add_argument('--lr', type=float, default=1e-5)  # 2e-4 for QM, 5e-5 for GEOM
...
parser.add_argument('--inv_sublayers', type=int, default=1,
                    help='number of layers')
parser.add_argument('--nf', type=int, default=128,  # 256 for QM, 192 for GEOM
...
parser.add_argument('--n_report_steps', type=int, default=800)
...
parser.add_argument('--generate_epochs', type=int, default=1,
                    help='save model')
parser.add_argument('--stop_test', type=int, default=200,
...
parser.add_argument('--start_epoch', type=int, default=350,
                    help='')
parser.add_argument('--ema_decay', type=float, default=0.999,  # 0.999 for QM, 0 for GEOM
...
parser.add_argument('--visualize_every_batch', type=int, default=1400,
                    help="Can be used to visualize multiple times per epoch")
parser.add_argument('--normalization_factor', type=float, default=1,  # 1 for QM, 100 for GEOM
...
if args.resume is not None:
    ...
    with open(join('ZDataU_Param/', args.resume, 'args.pickle'), 'rb') as f:
        args = pickle.load(f)
...
anion_context_node_nf = anion_context_dummy.size(2)
cation_context_node_nf = cation_context_dummy.size(2)
args.anion_context_node_nf = anion_context_node_nf
args.cation_context_node_nf = cation_context_node_nf
...
def main():
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        print(f"Epoch took {time.time() - start_epoch_:.1f} seconds.")
        ...
        if epoch % args.test_epochs == 0:
            ...
            if not args.break_train_epoch:
                analyze_and_save(args=args, epoch=epoch, model_sample=model_ema, device=device, prop_dist=prop_dist,
            ...
            if nll_val < best_nll_val or epoch == 0:
                ...
                if args.save_model:
                    ...
                    utils.save_model(optim, 'ZDataC_SavedModel/%s/optim.npy' % args.exp_name)
                    utils.save_model(model, 'ZDataC_SavedModel/%s/generative_model.npy' % args.exp_name)
                    if args.ema_decay > 0:
                        utils.save_model(model_ema, 'ZDataC_SavedModel/%s/generative_model_ema.npy' % args.exp_name)
                    with open('ZDataC_SavedModel/%s/args.pickle' % args.exp_name, 'wb') as f_:
                        pickle.dump(args, f_)
