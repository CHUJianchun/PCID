...
def single_bond_only(threshold, length, margin1_=5):
    if length < threshold + margin1_:
        return 1
    return 0


def geom_predictor(p, l_, limit_bonds_to_one=False):
    """ p: atom pair (couple of str)
        l: bond length (float)"""
    bond_order = get_bond_order(p[0], p[1], l_, check_exists=True)
