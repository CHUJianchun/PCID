...
from U_Chem.data_util.prepare.utils import download_data, cleanup_file
...
def download_dataset_md17(datadir, dataname, subset, splits=None, cleanup=True):
    ...
    if subset not in md17_subsets:
        logging.info(
            'Molecule {} not included in list of downloadable MD17 datasets! '
            'Attempting to download based directly upon input key.'.format(
                subset))
        md17_molecule = subset
    else:
        md17_molecule = md17_subsets[subset]

    # Define directory for which data_util will be output.
    ...
    logging.info(
        'Downloading and processing molecule {} from MD17 dataset. Output will be in directory: {}.'.format(subset,
                                                                                                            md17dir))
    ...
    # Convert raw MD17 data_util to torch tensors.
    ...
    for split, split_idx in splits.items():
        md17_data_split[split] = {key: val[split_idx] if type(val) is np.ndarray else val for key, val in
                                  md17_data.items()}

    # Save processed GDB9 data_util into train/validation/test splits
    logging.info('Saving processed data_util:')
...
def gen_splits_md17(num_pts):
    ...
    splits = {'train': torch.tensor(data_perm[mask_train]), 'valid': torch.tensor(data_perm[mask_valid]),
              'test': torch.tensor(data_perm[mask_test])}
