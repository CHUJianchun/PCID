...
def split_dataset(data, split_idxs):
    ...
    split_idxs :  dict
        ...
        values are the keys for the items in data_util that go into the split.
...
def process_xyz_files(data, process_file_fn, file_ext=None, file_idx_list=None, stack=True):
    """
    Take a set of datafiles and apply a predefined data_util processing script to each
    ...
    stack : bool, optional
        ?????
    """
    logging.info('Processing data_util file: {}'.format(data))
...
def process_xyz_md17(datafile):
    ...
    datafile : python file object
        File object containing the molecular data_util in the MD17 dataset.
