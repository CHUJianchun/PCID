...
from U_Sub_Structure.EGNN_model import EGNN_dynamics as models
from U_Sub_Structure.EDM_model import utils as diffusion_utils
from U_Sub_Structure.EDM_model import utils
...
def clip_noise_schedule(alphas2, clip_value=0.001):
    ...
    alphas_step = np.clip(alphas_step, a_min=clip_value, a_max=1.)  # 限幅至 clip_value ~ 1
    alphas2 = np.cumprod(alphas_step, axis=0)  # 一种累乘，https://blog.csdn.net/mimiduck/article/details/122772920
...
class PositiveLinear(torch.nn.Module):
    ...
    def reset_parameters(self) -> None:
        torch.nn.init.kaiming_uniform_(self.weight, a=math.sqrt(5))  # a = 2.236
...
class GammaNetwork(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.l2 = PositiveLinear(1, 1024)  # TODO 为啥这么宽？
        self.l3 = PositiveLinear(1024, 1)

        self.gamma_0 = torch.nn.Parameter(torch.tensor([-5.]))  # TODO 可能太大了，-5
        self.gamma_1 = torch.nn.Parameter(torch.tensor([10.]))  # TODO 可能太大了，10
        self.show_schedule()
...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def __init__(
            ...
            norm_biases=(0., 0., 0.), include_charges=True):
        super().__init__()
    ...
    def check_issues_norm_values(self, num_stdevs=8):
        ...
        # Checked if 1 / norm_value is still larger than 10 * standard deviation.
    ...
    def reconstruct(self, x, h, node_mask, edge_mask, context, t_int):
        s_int = t_int - 1
        s = s_int / self.T
        t = t_int / self.T
        ...
        z = alpha_t * xh + sigma_t * eps  # NOTE 加噪结果
        for s in reversed(range(0, t_int)):
            s_array = torch.full((1, 1), fill_value=s, device=z.device)
            t_array = s_array + 1
            s_array = s_array / self.T
            t_array = t_array / self.T
            z = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask, edge_mask, context, fix_noise=True)
        x, h = self.sample_p_xh_given_z0(z, node_mask, edge_mask, context, fix_noise=True)
        return x, h
    ...
    def log_pxh_given_z0_without_constants(
        ...
        # Compute sigma_0 and rescale to the integer scale of the data_util.
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always):
        ...
        # t0_always 在train的时候false
        # This part is about whether to include loss term 0 always.
        if t0_always:
            # loss_term_0 will be computed separately.
            # estimator = loss_0 + loss_t,  where t ~ U({1, ..., T}), NOTE test
            lowest_t = 1
        else:
            # estimator = loss_t,           where t ~ U({0, ..., T}), NOTE train
            lowest_t = 0
        ...
        # Compute gamma_s and gamma_t via the network. NOTE Appendix B
        gamma_s = self.inflate_batch_array(self.gamma(s), x)
        gamma_t = self.inflate_batch_array(self.gamma(t), x)
        ...
        # Sample zt ~ Normal(alpha_t x, sigma_t) , NOTE eps为采样, 或者说, 实际的噪声
        eps = self.sample_combined_position_feature_noise(
            n_samples=x.size(0), n_nodes=x.size(1), node_mask=node_mask)
        ...
        z_t = alpha_t * xh + sigma_t * eps  # NOTE 加噪结果
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None):
        ...
        """
        # Normalize data_util, take into account volume change in x.
    ...
    def sample_p_zs_given_zt(self, s, t, zt, node_mask, edge_mask, context, fix_noise=False):
        ...
        # NOTE max coord / node num < 0.7362
        if any((mu.max(dim=2)[0] / node_mask.sum(dim=1)).view(-1)) > 0.7362:
            mu /= (mu.max(dim=2)[0] / node_mask.sum(dim=1)).view(-1).max() / 0.7362
        # Compute sigma for p(zs | zt).
        sigma = sigma_t_given_s * sigma_s / sigma_t / 2
    ...
    def sample_combined_position_feature_noise(self, n_samples, n_nodes, node_mask):
        ...
        z_h = utils.sample_gaussian_with_mask(
            ...
            node_mask=node_mask)
        # NOTE 这两个都是高斯分布的采样，可能会有点大了，导致梯度爆炸
...
class ILEnVariationalDiffusion(torch.nn.Module):
    def __init__(
            self,
            anion_dynamics: models.EGNN_dynamics_QM9, anion_in_node_nf: int,
            cation_dynamics: models.EGNN_dynamics_QM9, cation_in_node_nf: int,
            n_dims: int, timesteps: int = 1000, parametrization='eps', noise_schedule='learned',
            noise_precision=1e-4, loss_type='vlb', norm_values=(1., 1., 1.),
            norm_biases=(None, 0., 0.), include_charges=True):
        super().__init__()
        self.anion_evd = EnVariationalDiffusion(anion_dynamics, anion_in_node_nf, n_dims,
                                                timesteps, parametrization, noise_schedule,
                                                noise_precision, loss_type, norm_values,
                                                norm_biases, include_charges)
        self.cation_evd = EnVariationalDiffusion(cation_dynamics, cation_in_node_nf, n_dims,
                                                 timesteps, parametrization, noise_schedule,
                                                 noise_precision, loss_type, norm_values,
                                                 norm_biases, include_charges)

    def forward(self, anion_x, anion_h, anion_node_mask=None, anion_edge_mask=None,
                cation_x=None, cation_h=None, cation_node_mask=None, cation_edge_mask=None,
                anion_context=None, cation_context=None):
        anion_neg_log_pxh = self.anion_evd(anion_x, anion_h, anion_node_mask, anion_edge_mask, anion_context)
        cation_neg_log_pxh = self.cation_evd(cation_x, cation_h, cation_node_mask, cation_edge_mask, cation_context)
        return anion_neg_log_pxh, cation_neg_log_pxh

    @torch.no_grad()
    def sample_chain(self, n_samples,
                     ...
                     keep_frames=None):
        return (
            self.anion_evd.sample_chain(
                n_samples=n_samples,
                n_nodes=anion_n_nodes, node_mask=anion_node_mask, edge_mask=anion_edge_mask, context=anion_context,
                keep_frames=keep_frames),
            self.cation_evd.sample_chain(
                n_samples=n_samples,
                n_nodes=cation_n_nodes, node_mask=cation_node_mask, edge_mask=cation_edge_mask, context=cation_context,
                keep_frames=keep_frames),
        )
