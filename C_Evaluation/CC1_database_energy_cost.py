...
for data_ in tqdm(data_list_):
    ...
    if len(data_[1]["components"]) == 1 and data_[1]["solvent"] is None:
        if (
            ...
            viscosity_name_list.append(data_[1]["components"][0]["name"])
        elif (
                data_[1]["title"]
            ...
            heat_capacity_name_list.append(data_[1]["components"][0]["name"])
...
for name in solubility_name_list:
    if name in viscosity_name_list and name in density_name_list and name in heat_capacity_name_list:
        full_property_name_list.append(name)
        solubility_sub_list = []
        viscosity_sub_list = []
        density_sub_list = []
        heat_capacity_sub_list = []
        ...
        for i in range(len(viscosity_list)):
            if viscosity_list[i][1]["components"][0]["name"] == name:
                viscosity_sub_list.append(viscosity_list[i][1])
        for i in range(len(density_list)):
            if density_list[i][1]["components"][0]["name"] == name:
                density_sub_list.append(density_list[i][1])
        for i in range(len(heat_capacity_list)):
            if heat_capacity_list[i][1]["components"][0]["name"] == name:
                heat_capacity_sub_list.append(heat_capacity_list[i][1])
...
for idx in range(len(full_property_list)):
    ...
    viscosity_sub_list = full_property[2]
    density_sub_list = full_property[3]
    heat_capacity_sub_list = full_property[4]
    data_point_list = []
    ...
    data_point_ = None
    data_point_list = []
    for viscosity_dataframe in viscosity_sub_list:
        temperature_index, value_index, pressure_index = None, None, None
        for i in range(len(viscosity_dataframe["dhead"])):
            try:
                if "Temperature" in viscosity_dataframe["dhead"][i][0]:
                    temperature_index = i
                elif "Viscosity" in viscosity_dataframe["dhead"][i][0]:
                    value_index = i
                elif (
                        "Pressure" in viscosity_dataframe["dhead"][i][0]
                        and "kPa" in viscosity_dataframe["dhead"][i][0]
                ):
                    pressure_index = i
            except TypeError:
                print(viscosity_dataframe["dhead"])
        ...
        try:
            ...
            print(viscosity_dataframe["dhead"])
            continue
        for point in viscosity_dataframe["data"]:
            temperature_ = float(point[temperature_index][0])
            pressure_ = float(
            ...
            data_point_ = [temperature_, pressure_, viscosity_]
            data_point_list.append(data_point_)
    ...
    poly_score = poly_reg.score(poly_x, data_point_list[:, -1])
    if linear_score > poly_score:
        regressed_dynamic_viscosity_335 = linear_reg.predict(np.array([335.64, 101.325]).reshape(1, -1))
        regressed_dynamic_viscosity_353 = linear_reg.predict(np.array([353.15, 101.325]).reshape(1, -1))
    else:
        regressed_dynamic_viscosity_335 = poly_reg.predict(
            poly_features.fit_transform(np.array([335.64, 101.325]).reshape(1, -1)))
        regressed_dynamic_viscosity_353 = poly_reg.predict(
            poly_features.fit_transform(np.array([353.15, 101.325]).reshape(1, -1)))
    """Density"""
    data_point_ = None
    data_point_list = []
    ...
    data_point_ = None
    data_point_list = []
    for heat_capacity_dataframe in heat_capacity_sub_list:
        temperature_index, value_index, pressure_index = None, None, None
        for i in range(len(heat_capacity_dataframe["dhead"])):
            try:
                if "Temperature" in heat_capacity_dataframe["dhead"][i][0]:
                    temperature_index = i
                elif "Heat capacity at constant pressure" in heat_capacity_dataframe["dhead"][i][0]:
                    value_index = i
                elif (
                        "Pressure" in heat_capacity_dataframe["dhead"][i][0]
                        and "kPa" in heat_capacity_dataframe["dhead"][i][0]
                ):
                    pressure_index = i
            except TypeError:
                print(heat_capacity_dataframe["dhead"])
        ...
        try:
            ...
            print(heat_capacity_dataframe["dhead"])
            continue
        for point in heat_capacity_dataframe["data"]:
            temperature_ = float(point[temperature_index][0])
            pressure_ = float(
                point[pressure_index][0]) if (pressure_index is not None and float(point[pressure_index][0]) - 101.325 > 0.5) else 101.325
            heat_capacity_ = float(point[value_index][0])
            data_point_ = [temperature_, pressure_, heat_capacity_]
            data_point_list.append(data_point_)
    if not data_point_list:
        full_reg_prop_list.append(None)
        continue
    data_point_list = np.array(data_point_list)
    if len(data_point_list) < 5:
        continue
    ...
    poly_score = poly_reg.score(poly_x, data_point_list[:, -1])
    if linear_score > poly_score:
        regressed_heat_capacity_335 = linear_reg.predict(np.array([335.64, 101.325]).reshape(1, -1))
        regressed_heat_capacity_353 = linear_reg.predict(np.array([353.15, 101.325]).reshape(1, -1))
    else:
        regressed_heat_capacity_335 = poly_reg.predict(
            poly_features.fit_transform(np.array([335.64, 101.325]).reshape(1, -1)))
        regressed_heat_capacity_353 = poly_reg.predict(
            poly_features.fit_transform(np.array([353.15, 101.325]).reshape(1, -1)))

    if all(np.array([il_x,
                     regressed_dynamic_viscosity_335[0],
                     regressed_dynamic_viscosity_353[0],
                     regressed_density_335[0],
                     regressed_density_353[0],
                     regressed_heat_capacity_335[0],
                     regressed_heat_capacity_353[0],
...
"""
def full_energy_cost(idx):
    print(f'[{idx}, {energy_cost(*full_reg_prop_list[idx])}],')


Parallel(n_jobs=-2, verbose=10)(
    delayed(full_energy_cost)(idx) for idx in range(len(full_reg_prop_list))
)
"""
full_reg_energy_cost_list = np.array([
    [7, 1.8016830914996371],
    [15, 7.265716547229951],
    [22, 1.6331763831402202],
    [2, 2.076366678715189],
    [12, 1.6222340360922645],
    [24, 2.395904170134817],
    [10, 2.4937412324942487],
    [19, 2.8649383592501545],
    [9, 2.146862361392932],
    [14, 1.8693829347338742],
    [20, 1.8732716069620572],
    [4, 2.1198755661230178],
    [1, 3.1783834474882333],
    [16, 3.9402615087111785],
    [0, 1.7287776615559771],
    [21, 2.2857683474174006],
    [5, 2.255925683814116],
    [6, 1.804450686573336],
    [18, 1.8797299600999193],
    [23, 1.625052502760301],
    [11, 1.5249502649835986],
    [13, 1.804715511789227],
    [17, 2.519049167378338],
    [8, 1.6611708948717607],
    [3, 2.8382249884200323],
])

full_reg_energy_cost_list = full_reg_energy_cost_list[np.argsort(full_reg_energy_cost_list[:, 1])]
mol_for_df = []
energy_cost_for_df = []
name_for_df = []
smiles_for_df = []
...
for i in range(len(full_reg_energy_cost_list)):
    mol_for_df.append(
        Chem.MolFromSmiles(full_reg_smiles_list[int(full_reg_energy_cost_list[i, 0])]))
    energy_cost_for_df.append(full_reg_energy_cost_list[i, 1])
    smiles_for_df.append(full_reg_smiles_list[int(full_reg_energy_cost_list[i, 0])])
    name_for_df.append(full_reg_name_list[int(full_reg_energy_cost_list[i, 0])])
    properties_for_df.append(full_reg_prop_list[int(full_reg_energy_cost_list[i, 0])])
...
df['Dynamic viscosity in 335.64K'] = properties_for_df[:, 1]
df['Dynamic viscosity in 353.14K'] = properties_for_df[:, 2]
df['Density in 335.64K'] = properties_for_df[:, 3]
df['Density in 353.14K'] = properties_for_df[:, 4]
df['Heat Capacity in 335.15K'] = properties_for_df[:, 5]
df['Heat Capacity in 353.14K'] = properties_for_df[:, 6]
df['Solubility in 335.64K, 3Mpa'] = properties_for_df[:, 7]
df['Solubility in 353.15K'] = properties_for_df[:, 8]
df['Solubility in 339.14K'] = properties_for_df[:, 9]
df['Mass solubility difference'] = (properties_for_df[:, 7] - properties_for_df[:, 8]) * 44 / properties_for_df[:, 0]
# df = df.sort_values(by='Mass solubility difference', ascending=False)
PandasTools.AddMoleculeColumnToFrame(df, 'Smiles', 'Figure')
PandasTools.SaveXlsxFromFrame(df, 'ZDataE_AnalysisResult/database_energy_consumption.xlsx', molCol='Figure')
# 2.1724696390624176 for BMIM BF4
