...
class Ion:
    def __init__(self, smiles_, type_):
        if type_ == "anion":
            ion_max_atom_num = 34
        elif type_ == "cation":
            ion_max_atom_num = 67
        else:
            raise ValueError("Impossible ion type")
        mol = Chem.MolFromSmiles(smiles_)
        self.m = Descriptors.MolWt(mol)
        self.smiles = smiles_
        self.num_atoms = mol.GetNumAtoms()
        self.mol_charge = np.zeros(ion_max_atom_num)
        self.mol_coordinate = np.zeros((ion_max_atom_num, 3))
        mol = Chem.AddHs(mol)
        Chem.Kekulize(mol)
        ...
        self.mol_charge = np.pad(self.mol_charge, (0, ion_max_atom_num - len(self.mol_charge)),
                                 'constant', constant_values=0)

        self.one_hot = np.pad(self.one_hot, ((0, ion_max_atom_num - len(self.one_hot)), (0, 0)),
                              'constant', constant_values=0)
        self.mol_coordinate = np.pad(self.mol_coordinate,
                                     ((0, ion_max_atom_num - len(self.mol_coordinate)), (0, 0)),
                                     'constant', constant_values=0)

        self.atom_mask = self.mol_charge != 0
        self.atom_mask.astype(np.float64)
        self.edge_mask = np.ones((ion_max_atom_num, ion_max_atom_num)) - np.eye(ion_max_atom_num)
...
def prepare_regress_dataset():
    ...
    cation_smiles_cache = []
    cation_mol_cache = []
    bad_cation_cache = ["Cl[In+2]"]

    def prepare_regress_dataset_for(property_):
        out_file_path = "ZDataB_ProcessedData/property_" + property_ + "_separated_dataset.npz"
        data_point_list = unpickle_data_point_list(property_)
        anion_charge_list = []
        anion_atom_position_list = []
        anion_node_mask_list = []
        anion_edge_mask_list = []
        anion_n_nodes_list = []
        anion_one_hot_list = []
        cation_charge_list = []
        cation_atom_position_list = []
        cation_node_mask_list = []
        cation_edge_mask_list = []
        cation_n_nodes_list = []
        cation_one_hot_list = []
        temperature_list = []
        pressure_list = []
        ...
        for data_point in tqdm(data_point_list):
            if 70 < data_point[1] < 5000:
                if data_point[3].split(".")[0] not in bad_anion_cache and data_point[3].split(
                        ".")[1] not in bad_cation_cache:
                    if data_point[3].split(".")[0] not in anion_smiles_cache:
                        anion = Ion(data_point[3].split(".")[0], type_="anion")
                        if anion.return_value == 0:
                            anion_smiles_cache.append(data_point[3].split(".")[0])
                            anion_mol_cache.append(anion)
                        else:
                            bad_anion_cache.append(data_point[3].split(".")[0])
                            continue
                    else:
                        anion = anion_mol_cache[anion_smiles_cache.index(data_point[3].split(".")[0])]

                    if data_point[3].split(".")[1] not in cation_smiles_cache:
                        cation = Ion(data_point[3].split(".")[1], type_="cation")
                        if cation.return_value == 0:
                            cation_smiles_cache.append(data_point[3].split(".")[1])
                            cation_mol_cache.append(cation)
                        else:
                            bad_cation_cache.append(data_point[3].split(".")[1])
                            continue
                    else:
                        cation = cation_mol_cache[cation_smiles_cache.index(data_point[3].split(".")[1])]

                    anion_charge_list.append(anion.mol_charge)
                    anion_atom_position_list.append(anion.mol_coordinate)
                    anion_node_mask_list.append(anion.atom_mask)
                    anion_edge_mask_list.append(anion.edge_mask)
                    anion_n_nodes_list.append(anion.num_atoms)
                    anion_one_hot_list.append(anion.one_hot)

                    cation_charge_list.append(cation.mol_charge)
                    cation_atom_position_list.append(cation.mol_coordinate)
                    cation_node_mask_list.append(cation.atom_mask)
                    cation_edge_mask_list.append(cation.edge_mask)
                    cation_n_nodes_list.append(cation.num_atoms)
                    cation_one_hot_list.append(cation.one_hot)

                    temperature_list.append(data_point[0])
                    pressure_list.append(data_point[1])
                    value_list.append(data_point[2])

        anion_charge_list = np.array(anion_charge_list)
        anion_atom_position_list = np.array(anion_atom_position_list)
        anion_node_mask_list = np.array(anion_node_mask_list)
        anion_edge_mask_list = np.array(anion_edge_mask_list)
        anion_n_nodes_list = np.array(anion_n_nodes_list)

        cation_charge_list = np.array(cation_charge_list)
        cation_atom_position_list = np.array(cation_atom_position_list)
        cation_node_mask_list = np.array(cation_node_mask_list)
        cation_edge_mask_list = np.array(cation_edge_mask_list)
        cation_n_nodes_list = np.array(cation_n_nodes_list)
        ...
        np.savez(out_file_path,
                 anion_charge_list=anion_charge_list,
                 anion_atom_position_list=anion_atom_position_list,
                 anion_node_mask_list=anion_node_mask_list,
                 anion_edge_mask_list=anion_edge_mask_list,
                 anion_n_nodes_list=anion_n_nodes_list,
                 anion_one_hot_list=anion_one_hot_list,

                 cation_charge_list=cation_charge_list,
                 cation_atom_position_list=cation_atom_position_list,
                 cation_node_mask_list=cation_node_mask_list,
                 cation_edge_mask_list=cation_edge_mask_list,
                 cation_n_nodes_list=cation_n_nodes_list,
                 cation_one_hot_list=cation_one_hot_list,
...
def prepare_all_il_dataset():
    out_file_path = "ZDataB_ProcessedData/all_il_separated_dataset.npz"
    anion_charge_list = []
    anion_atom_position_list = []
    anion_node_mask_list = []
    anion_edge_mask_list = []
    ...
    anion_mw_list = []
    cation_mw_list = []
    anion_smiles_list = unpickle_ion_smiles_list("Anion")
    cation_smiles_list = unpickle_ion_smiles_list("Cation")
    anion_mol_list = []
    cation_mol_list = []
    save_anion_smiles_list = []
    save_cation_smiles_list = []
    for anion_smiles in anion_smiles_list:
        anion_mol = Ion(anion_smiles, "anion")
        if anion_mol.return_value == 0:
            anion_mol_list.append(anion_mol)

    for cation_smiles in cation_smiles_list:
        if cation_smiles != "Cl[In+2]":
            cation_mol = Ion(cation_smiles, "cation")
            if cation_mol.return_value == 0:
                cation_mol_list.append(cation_mol)

    for anion in anion_mol_list:
        anion_charge_list.append(anion.mol_charge)
        anion_atom_position_list.append(anion.mol_coordinate)
        anion_node_mask_list.append(anion.atom_mask)
        anion_edge_mask_list.append(anion.edge_mask)
        anion_n_nodes_list.append(anion.num_atoms)
        anion_one_hot_list.append(anion.one_hot)
        anion_mw_list.append(anion.m)
        save_anion_smiles_list.append(anion.smiles)
    for cation in cation_mol_list:
        cation_charge_list.append(cation.mol_charge)
        cation_atom_position_list.append(cation.mol_coordinate)
        cation_node_mask_list.append(cation.atom_mask)
        cation_edge_mask_list.append(cation.edge_mask)
        cation_n_nodes_list.append(cation.num_atoms)
        cation_one_hot_list.append(cation.one_hot)
        cation_mw_list.append(cation.m)
        save_cation_smiles_list.append(cation.smiles)
    ...
    anion_edge_mask_list = np.array(anion_edge_mask_list)
    anion_n_nodes_list = np.array(anion_n_nodes_list)
    anion_mw_list = np.array(anion_mw_list)
    anion_smiles_list = np.array(save_anion_smiles_list)
    ...
    cation_edge_mask_list = np.array(cation_edge_mask_list)
    cation_n_nodes_list = np.array(cation_n_nodes_list)
    cation_mw_list = np.array(cation_mw_list)
    cation_smiles_list = np.array(save_cation_smiles_list)
    ...
    np.savez(out_file_path,
             ...
             anion_n_nodes_list=anion_n_nodes_list,
             anion_one_hot_list=anion_one_hot_list,
             anion_mw_list=anion_mw_list,
             anion_smiles_list=anion_smiles_list,
             ...
             cation_n_nodes_list=cation_n_nodes_list,
             cation_one_hot_list=cation_one_hot_list,
             cation_mw_list=cation_mw_list,
             cation_smiles_list=cation_smiles_list
             )
